#!/bin/bash
MONERO_URL=https://github.com/flakechain/flake-core.git
...
MONERO_DIR=$ROOT_DIR/flake-core
BUILD_LIBWALLET=false

# init and update monero submodule
if [ ! -d $MONERO_DIR/src ]; then
    git submodule init flake-core
fi
...
git -C $MONERO_DIR config user.name "Flake GUI"
git -C $MONERO_DIR config user.email "gui@flake.local"
# check for PR requirements in most recent commit message (i.e requires #xxxx)
for PR in $(git log --format=%B -n 1 | grep -io "requires #[0-9]*" | sed 's/[^0-9]*//g'); do
    echo "Merging flake push request #$PR"
    # fetch pull request and merge
    git -C $MONERO_DIR fetch origin pull/$PR/head:PR-$PR
    git -C $MONERO_DIR merge --quiet PR-$PR  -m "Merge flake PR #$PR"
...
## Compare previously built version with submodule + merged PR's version. 
    ...
    if [ "$VERSIONTAG" != "$GUI_MONERO_VERSION" ]; then
        ...
        echo "latest libwallet ($GUI_MONERO_VERSION) is already built. Remove flake-core/lib/libwallet_merged.a to force rebuild"
    fi
fi
